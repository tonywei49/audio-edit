(function (w, d, PKAE) {
	...
	function _makeUITopHeader(menu_tree, UI) {
		...
		function build_menus(parent_el, tree_obj, level) {
			for (var i = 0; i < tree_obj.length; ++i) {
				...
				if (level === 0) {
					...
					// i18n support
					var lowerName = curr_obj.name.toLowerCase();
					if (['file', 'edit', 'effects', 'view', 'help'].indexOf(lowerName) > -1) {
						btn.setAttribute('data-i18n', 'menu_' + lowerName);
					}
		...
		var langContainer = d.createElement('div');
		langContainer.className = 'pk_btn pk_noselect';
		// langContainer.style.float = 'left'; // Already floated by class

		var langSelect = d.createElement('select');
		langSelect.id = 'lang-select';
		langSelect.className = 'pk_lang_select';
